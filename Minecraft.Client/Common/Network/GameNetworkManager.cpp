...
#ifdef _DEDICATED_SERVER
// Dedicated server entry point - runs server on main thread
int CGameNetworkManager::DedicatedServerMain( void* lpParameter )
{
	__int64 seed = 0;
	if(lpParameter != NULL)
	{
		NetworkGameInitData *param = (NetworkGameInitData *)lpParameter;
		seed = param->seed;
		DedicatedServerApp::GetInstance()->SetGameHostOption((eDedicatedGameHostOption)eGameHostOption_All, param->settings);
	}

	// Initialize thread storage for main thread
	AABB::CreateNewThreadStorage();
	Vec3::CreateNewThreadStorage();
	IntCache::CreateNewThreadStorage();
	Compression::UseDefaultThreadStorage();
	...
	int result = MinecraftServer::mainDedicated(seed, lpParameter);
