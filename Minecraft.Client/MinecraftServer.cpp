...
#ifdef _DEDICATED_SERVER
	HeadlessProgressRenderer *mcprogress = DedicatedServerApp::GetInstance()->GetProgressRenderer();
#else
...
void MinecraftServer::saveAllChunks()
{
	...
	for (unsigned int i = 0; i < levels.length; i++)
	{
		...
		{
#ifdef _DEDICATED_SERVER
			level->save(true, DedicatedServerApp::GetInstance()->GetProgressRenderer());
#else
...
void MinecraftServer::stopServer(bool didInit)
{
...
#endif
	{
		...
		if(m_saveOnExit && ProfileManager.IsFullVersion() && (!StorageManager.GetSaveDisabled()) && didInit)
		{
			...
			{
#ifdef _DEDICATED_SERVER
				players->saveAll(DedicatedServerApp::GetInstance()->GetProgressRenderer(), true);
#else
			...
			{
#ifdef _DEDICATED_SERVER
				levels[0]->saveToDisc(DedicatedServerApp::GetInstance()->GetProgressRenderer(), false);
#else
...
//					PIXReportCounter(L"Server ticks",(float)tickcount);
			...
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
...
#ifdef _DEDICATED_SERVER
							levels[0]->saveToDisc(DedicatedServerApp::GetInstance()->GetProgressRenderer(), true);
#else
				...
				case eXuiServerAction_SaveGame:
					...
					{
#ifdef _DEDICATED_SERVER
						players->saveAll(DedicatedServerApp::GetInstance()->GetProgressRenderer());
#else
					...
					for (unsigned int j = 0; j < levels.length; j++)
					{
						...
						ServerLevel *level = levels[levels.length - 1 - j];
#ifdef _DEDICATED_SERVER
						level->save(true, DedicatedServerApp::GetInstance()->GetProgressRenderer(), (eAction==eXuiServerAction_AutoSaveGame));
#else
...
#ifdef _DEDICATED_SERVER
						levels[0]->saveToDisc(DedicatedServerApp::GetInstance()->GetProgressRenderer(), (eAction==eXuiServerAction_AutoSaveGame));
#else
...
#ifdef _DEDICATED_SERVER
// Entry point for dedicated server - runs on main thread, no graphics
int MinecraftServer::mainDedicated(__int64 seed, void *lpParameter)
{
	DedicatedServerApp::GetInstance()->DebugPrintf("Starting dedicated server main thread\n");

	server = new MinecraftServer();
	server->run(seed, lpParameter);

	int result = server->stopped ? 0 : 1;
